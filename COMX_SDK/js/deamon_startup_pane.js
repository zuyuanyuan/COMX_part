...
function OnInitializeData()
{
    ...
    binding.prog_val = 'play';

	SetTimeout(1000, function(nid_main){
		if(comx.shell.IsWebSocketPortValid(comx.gf.deamon_port) == "true" && flag == false)
		{
			KillTimeout(nid_main);
			flag = true;
			comx.sys.RunJavaScriptCodes("websocket.ConnectTo(comx.gf.deamon_port);", comx.gf.mainwindow_formid);
            //pane.Close();
            SetTimeout(1000, function(nid){
				KillTimeout(nid);
                //gif.Stop(progress_timer);
                binding.prog_val = 'stop';
				pane.Close();
			});
        }
	});
