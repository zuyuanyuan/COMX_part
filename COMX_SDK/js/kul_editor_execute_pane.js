function OnInitializeData()
{
    ...
    var config_fname = comx.sys.GetEnvironmentVariables('COMX_SDK') + '/data/urun.json';
    if(comx.sys.IsFileExist(config_fname))
    {
        comx.sys.CreateFileHandler('urun_json', config_fname);
        var cfg_str = comx.file.urun_json;
        comx.sys.CloseFileHandler('urun_json');
        var cfg_obj = JSON.parse(cfg_str);
        binding.unit_index = cfg_obj.unit_index;
    }
...
{
    var content = {'unit_index' : binding.unit_index};
    var config_fname = comx.sys.GetEnvironmentVariables('COMX_SDK') + '/data/urun.json';
    comx.sys.CreateFileHandler('urun_json', config_fname);
    comx.file.urun_json = JSON.stringify(content);
    comx.sys.CloseFileHandler('urun_json');
}