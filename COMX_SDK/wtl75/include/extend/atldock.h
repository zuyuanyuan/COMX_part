...
public:
   ...
   {
      m_cxySplitter = static_cast<long>(wParam);
      return 0;
   }
...
public:
   ...
   void SetWindowRect(HWND hWnd, const CRect &rc)
   {
	   T* pT = static_cast<T*>(this);
	   DOCKCONTEXT* pCtx = _GetContext(hWnd);
	   ATLASSERT(pCtx);
	   ...
	   pCtx->sizeFloat.cx = rc.Width();
	   pCtx->sizeFloat.cy = rc.Height();
   ...
   //{
	  ...
	  // if( IsFloating(pCtx->Side) ) return pT->_FloatWindow(pCtx);
	  // else if( IsDocked(pCtx->Side) ) return pT->_DockWindow(pCtx,pCtx->Side,GetPaneSize(pCtx->Side), TRUE);
	  // return TRUE;
   //}
