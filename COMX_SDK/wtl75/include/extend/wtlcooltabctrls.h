...
public:
   ...
   int m_iCurSel;                             // Index of currently selected item
   ...
   int HitTest(LPTCHITTESTINFO pinfo) const
   {
      ...
      // Hit test selected item first, since it may be larger than other buttons
      int iCurSel = GetCurSel();
      if( iCurSel >= 0 ) {
         if( ::PtInRect(&m_Items[iCurSel]->rcItem, pinfo->pt) ) {
            pinfo->flags = TCHT_ONITEM;
            return iCurSel;
         }
      }
      // Hit test other buttons
   ...
   void DoPaint(CDCHandle dc, RECT &rcClip)
   {
      ...
      if( (GetExStyle() & WS_EX_TRANSPARENT) == 0 ) dc.FillRect(&rcClient, ::GetSysColorBrush(COLOR_3DFACE));
...
public:
   ...
   void _GetTrapezoidPoints(const RECT& rc, POINT* pts) const
   {
      pts[0].x = rc.left;
      pts[0].y = rc.top;
