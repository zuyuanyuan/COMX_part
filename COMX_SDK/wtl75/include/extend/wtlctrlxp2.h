...
public:
	CToolBarXPCtrl(HWND hWnd = NULL)
	{ 
		m_hWnd = hWnd;
	}

	CToolBarXPCtrl& operator =(HWND hWnd)
	{
		m_hWnd = hWnd;
		return *this;
	}
...
public:
	inline BOOL LoadTrueColorToolBar(int nBtnWidth, UINT uToolBar, UINT uToolBarHot, UINT uToolBarDisabled = 0, HINSTANCE hInst = NULL)
	{
		if (!SetTrueColorToolBar(TB_SETIMAGELIST, uToolBar, nBtnWidth, hInst))
			return FALSE;

		if (!SetTrueColorToolBar(TB_SETHOTIMAGELIST, uToolBarHot, nBtnWidth, hInst))
			return FALSE;

		if (uToolBarDisabled) {
			if (!SetTrueColorToolBar(TB_SETDISABLEDIMAGELIST, uToolBarDisabled, nBtnWidth, hInst))
				return FALSE;
		}
	...
	inline BOOL LoadTrueColorToolBar(CImageList &uToolBar, CImageList &uToolBarHot, CImageList &uToolBarDisabled)
	{
		if (!SetTrueColorToolBar(TB_SETIMAGELIST, uToolBar))
			return FALSE;

		if (!SetTrueColorToolBar(TB_SETHOTIMAGELIST, uToolBarHot))
			return FALSE;

		if (uToolBarDisabled) {
			if (!SetTrueColorToolBar(TB_SETDISABLEDIMAGELIST, uToolBarDisabled))
				return FALSE;
		}
...
private:
	inline BOOL SetTrueColorToolBar(UINT uToolBarType, UINT uToolBar, int nBtnWidth, HINSTANCE hInst = NULL)
	{
		CImageList	cImageList = GenerateImageListFromID(uToolBar , nBtnWidth, hInst);
		SendMessage(uToolBarType, 0, (LPARAM)cImageList.m_hImageList);

		return TRUE;
	}
	inline BOOL SetTrueColorToolBar(UINT uToolBarType, CImageList &uToolBar)
	{
		SendMessage(uToolBarType, 0, (LPARAM)uToolBar.m_hImageList);
